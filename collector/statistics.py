...
class StatisticsCalculator:
    ...
    def calculate_statistics(self):
        """Calculate namespace-level statistics from the last 7 days of data."""
    ...
    def get_pod_anomalies(self, namespace=None):
        """
        Pod-level anomaly detection.
        Compares each pod's restart count and CPU request
        against the namespace average.
        """
        ...
        for metric in recent:
            ...
            for pod in metric.pod_data:
                ...
                # CPU anomaly — more than 3x the namespace average
                cpu_ratio = pod_cpu / max(ns_avg_cpu, 0.001)
                cpu_score = min(100.0, max(0.0, (cpu_ratio - 1) * 30))

                # Restart anomaly — 5+ restarts is high risk
                restart_score = min(100.0, restarts * 10.0)

                # Overall score — weighted average of both
