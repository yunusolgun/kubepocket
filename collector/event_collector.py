...
class EventCollector:
    ...
    def collect_events(self):
        """
        Collect critical events from all namespaces and save to DB.
        Updates count instead of inserting duplicate (pod, event_type) records.
        """
        db = SessionLocal()
        saved = 0
        updated = 0
        ...
        try:
            ...
            for event in events.items:
                ...
                # Update existing record if same pod + event_type exists
                existing = (
                    db.query(KubeEvent)
                    .filter(
    ...
    def get_event_summary(self, hours=24):
        """Return event summary for the last X hours."""
