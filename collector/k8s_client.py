...
class K8sClient:
    def __init__(self, context=None):
        try:
            ...
            try:
                ...
                print(
                    f"✅ Kubeconfig loaded (context: {context or 'default'})")
        ...
        try:
            ...
            print("⚠️ Metrics API not available")

    def parse_cpu(self, cpu_str):
        if not cpu_str:
            return 0
        if cpu_str.endswith('n'):      # nanoseconds — used by Metrics Server
    ...
    def collect_pvc_metrics(self):
        ...
        try:
            ...
            for pvc in pvcs.items:
                ...
                # Actual usage from Prometheus node-exporter
                actual_gib = None
                used_pct = None
                try:
            ...
            print(f"❌ PVC metrics error: {e}")
            return []

    def collect_node_metrics(self):
        """Node capacity, allocatable resources and pod distribution."""
        try:
    ...
    def collect_all_metrics_with_usage(self):
        """collect_all_metrics() + actual usage from Metrics Server."""
