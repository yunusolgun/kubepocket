...
#   TEAMS_WEBHOOK_URL     Microsoft Teams Incoming Webhook URL (default: empty)
#   WEBHOOK_URL           Generic HTTP POST endpoint (default: empty)
#   WEBHOOK_MIN_SEVERITY  Minimum severity to notify: warning | critical (default: warning)
#
# WEEKLY REPORT
#   WEEKLY_REPORT_DAY     Day of week: 0=Mon ... 6=Sun (default: 0)
#   WEEKLY_REPORT_HOUR    Hour in UTC 0-23 (default: 9)
#
# SECURITY NOTE
...
#
# USAGE
#   ./install.sh              — install KubePocket
#   ./install.sh uninstall    — remove KubePocket (keeps monitoring stack)
...
if [ "${1:-}" = "uninstall" ]; then
  ...
  echo "🗑️  Uninstalling KubePocket..."

  helm uninstall kubepocket -n kubepocket 2>/dev/null &&     echo "✅ KubePocket Helm release removed" ||     echo "⚠️  KubePocket release not found (already removed?)"

  kubectl delete namespace kubepocket 2>/dev/null &&     echo "✅ Namespace 'kubepocket' deleted" ||     echo "⚠️  Namespace 'kubepocket' not found"

  if [ "$REMOVE_ALL" = "1" ]; then
    echo "🗑️  Removing monitoring stack..."
    helm uninstall monitoring -n monitoring 2>/dev/null &&       echo "✅ Monitoring Helm release removed" ||       echo "⚠️  Monitoring release not found"

    kubectl delete namespace monitoring 2>/dev/null &&       echo "✅ Namespace 'monitoring' deleted" ||       echo "⚠️  Namespace 'monitoring' not found"
  else
    echo "ℹ️  Monitoring stack kept. Use './install.sh uninstall --all' to remove it too."
  fi
...
WEBHOOK_MIN_SEVERITY=${WEBHOOK_MIN_SEVERITY:-warning}
WEEKLY_REPORT_DAY=${WEEKLY_REPORT_DAY:-0}
...
[ -n "$SLACK_WEBHOOK_URL" ] && echo "   Slack Webhook:      configured" || true
echo ""
...
helm install kubepocket ./helm/kubepocket \
  ...
  $LICENSE_FLAG \
  ${SLACK_WEBHOOK_URL:+--set webhook.slackUrl="$SLACK_WEBHOOK_URL"} \
  ${TEAMS_WEBHOOK_URL:+--set webhook.teamsUrl="$TEAMS_WEBHOOK_URL"} \
  ${WEBHOOK_URL:+--set webhook.genericUrl="$WEBHOOK_URL"} \
  --set webhook.minSeverity="$WEBHOOK_MIN_SEVERITY" \
  --set webhook.weeklyReport.day="$WEEKLY_REPORT_DAY" \
  --set webhook.weeklyReport.hour="$WEEKLY_REPORT_HOUR" \
  --wait --timeout "$HELM_TIMEOUT"
