...
| `WEBHOOK_URL`          | *(empty)*                   | Generic HTTP POST endpoint                             |
| `WEBHOOK_MIN_SEVERITY` | `warning`                   | Minimum alert severity: `warning` or `critical`        |
| `WEEKLY_REPORT_DAY`    | `0`                         | Weekly report day: 0=Mon … 6=Sun                       |
| `WEEKLY_REPORT_HOUR`   | `9`                         | Weekly report hour in UTC (0–23)                       |
...
A weekly summary report is automatically sent to all configured webhook providers. It runs every Monday at 09:00 UTC by default and includes:

- **Cluster overview** — namespace count, pod count, total CPU/memory
- **Trend vs previous week** — CPU and memory usage change (%)
- **Top 5 waste pods** — pods wasting the most resources
- **Top 5 anomaly pods** — pods with highest anomaly scores
- **Most restarted pods** — top 5 by restart count
- **Active alerts** — critical / warning counts and new alerts this week
- **PVC issues** — unbound or near-full (≥75%) PVCs
- **Longest running pods** — top 5 by uptime

The report is sent automatically when the collector daemon is running and a webhook URL is configured. No additional setup is required.
...
helm upgrade kubepocket ./helm/kubepocket \
  --namespace kubepocket \
  --reuse-values \
  --set webhook.weeklyReport.day=4 \   # 0=Mon, 1=Tue, ..., 6=Sun
  --set webhook.weeklyReport.hour=8     # UTC hour (0-23)
```

**Trigger manually:**

```bash
kubectl exec -n kubepocket deploy/kubepocket -- python3 -c "
import sys; sys.path.insert(0, '/app')
from db.models import SessionLocal
from collector.weekly_report import send_weekly_report
db = SessionLocal()
...
| values.yaml key                    | Environment Variable                  | Default | Description              |
|------------------------------------|---------------------------------------|---------|--------------------------|
| `webhook.weeklyReport.day`         | `KUBEPOCKET_WEEKLY_REPORT_DAY`        | `0`     | Day of week (0=Mon–6=Sun)|
| `webhook.weeklyReport.hour`        | `KUBEPOCKET_WEEKLY_REPORT_HOUR`       | `9`     | Hour in UTC (0–23)       |

> **Note:** Trend percentages require at least two weeks of data to be meaningful. On fresh installations the trend will show `+0.0%`.
...
```bash
# Remove KubePocket only (keeps Prometheus + Grafana)
./install.sh uninstall

# Remove KubePocket and the monitoring stack
./install.sh uninstall --all
```

The uninstall command removes the Helm release and the `kubepocket` namespace. Your PostgreSQL data is deleted along with the namespace. The monitoring stack (Prometheus + Grafana) is kept by default unless `--all` is passed.
